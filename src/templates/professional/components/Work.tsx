...
import { dateParser, numberOfYears } from 'src/helpers/utils';
  ...
  return (
    <div>
      {work.map((company, index) => (
        <Timeline key={`${company.name}-${index}`} className="p-0 flex-initial my-2 text-[1em]">
...
function ExperienceHeader({ work }: { work: IExperienceItem }) {
  const experienceYears = numberOfYears(work.startDate, work.endDate);
  console.log('Rahul: experienceYears:', work.startDate, work.endDate, experienceYears);
  return (
    <>
      ...
      <div className="flex justify-between items-end">
        ...
        <div className="italic text-xs">{experienceYears}</div>
      </div>
    </>
  );
}
